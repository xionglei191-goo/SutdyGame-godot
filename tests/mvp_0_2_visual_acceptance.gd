...
func _assert_world_overview_layer_map(town_map: Node) -> void:
	var previous_scene_id: String = town_map.get_active_scene() if town_map.has_method("get_active_scene") else ""
	var world_scene: Node = town_map.get_scene_root("world_overview")
	var ground: CanvasItem = town_map.get_node("Ground")
	...
	_assert(ground.z_index < world_scene.z_index + layer_map.z_index, "scene ground should stay behind the world overview layer map")
	...
	_assert(layer_map.get_landmark_object_count() >= 16, "world overview should reserve independent landmark object slots for primary town and transport places")
	_assert(layer_map.has_method("get_place_marker_count"), "layer map renderer should expose data-derived place marker count")
	_assert(layer_map.get_place_marker_count() >= 22, "world overview layer map should draw default and near-term place markers without the reference PNG")
	_assert(layer_map.has_method("get_memory_anchor_marker_count"), "layer map renderer should expose data-derived memory anchor marker count")
	_assert(layer_map.get_memory_anchor_marker_count() == 26, "world overview layer map should draw the full A-Z memory-palace marker layer")
	_assert(layer_map.has_method("get_asset_generation_mode"), "layer map renderer should expose landmark asset generation mode")
	_assert(layer_map.get_asset_generation_mode() == "built_in_imagegen_with_local_script_fallback", "landmark art should prefer built-in imagegen and record the local fallback")
	_assert(layer_map.has_method("get_asset_generation_preferred_tool"), "layer map renderer should expose preferred landmark generation tool")
	_assert(layer_map.get_asset_generation_preferred_tool() == "built_in_imagegen", "landmark art should keep built-in imagegen as the preferred tool")
	_assert(layer_map.has_method("get_asset_generation_current_source"), "layer map renderer should expose the current landmark generation source")
	var current_source: String = str(layer_map.get_asset_generation_current_source())
	_assert(current_source in ["built_in_imagegen", "local_fallback"], "landmark art should record whether assets came from built-in generation or local fallback")
	_assert(layer_map.has_method("has_loaded_tile_atlas"), "layer map renderer should expose tile atlas load status")
	_assert(layer_map.has_loaded_tile_atlas(), "world overview tile layers should use the generated tile atlas PNG")
	_assert(layer_map.get_tile_atlas_path() == "res://assets/generated/maps/world/tiles/tile_atlas_world_v001.png", "world overview should load the generated tile atlas path")
	_assert(layer_map.get_tile_atlas_status() == "generated_programmatic", "world overview tile atlas should be marked generated")
	_assert(layer_map.has_method("get_grid_rule_style_goal"), "layer map renderer should expose the life-grid style goal")
	_assert(layer_map.get_grid_rule_style_goal() == "continuous_player_on_discrete_life_grid", "world overview should use a continuous-movement life grid contract")
	_assert(layer_map.get_grid_rule_movement_mode() == "continuous", "world overview player movement should remain continuous over the grid")
	_assert(layer_map.grid_rules_snap_interactions_to_grid(), "world overview interactions should snap to stable grid cells")
	_assert(layer_map.get_grid_cell_size() == 128, "world overview life grid should match the 128px tile cells")
	_assert(layer_map.get_grid_district_count() >= 3, "world overview life grid should define home, school, and town districts")
	_assert(layer_map.get_grid_occupied_cell_count() >= 24, "world overview life grid should reserve occupied landmark cells")
	_assert(layer_map.get_grid_interaction_cell_count() >= 10, "world overview life grid should define stable interaction cells")
	_assert(layer_map.get_grid_road_cell_count() >= 32, "world overview life grid should expose connected road cells")
	_assert(layer_map.get_grid_interaction_cell_for_hotspot("home") == Vector2i(2, 7), "home should keep a grid interaction cell near the starting road")
	_assert(layer_map.get_grid_interaction_cell_for_hotspot("bus_station") == Vector2i(1, 11), "bus station should keep a grid interaction cell on the transport road")
	_assert(layer_map.get_grid_district_id_for_cell(9, 8) == "school_footprint", "school interaction cells should stay inside the school footprint")
	_assert(layer_map.get_grid_district_id_for_cell(5, 14) == "town_shop_street", "starter shop interactions should sit on the town shop street")
	_assert(layer_map.is_grid_cell_occupied(1, 6), "home building cells should be occupied")
	_assert(not layer_map.is_grid_cell_walkable(1, 6), "occupied building cells should not be walkable")
	_assert(layer_map.is_grid_cell_walkable(2, 7), "home road interaction cell should stay walkable")
	_assert(town_map.has_method("get_world_grid_collision_count"), "scene host should expose generated world grid collision count")
	town_map.show_scene("world_overview")
	_assert(town_map.get_world_grid_collision_count() == layer_map.get_grid_occupied_rects().size(), "world overview should generate collision bodies from occupied life-grid cells")
	town_map.show_scene("home")
	_assert(town_map.get_world_grid_collision_count() == 0, "world overview grid collisions should be cleared outside the world map")
	town_map.show_scene("world_overview")
	_assert(layer_map.has_method("get_generated_landmark_asset_count"), "layer map renderer should expose generated landmark count")
	_assert(layer_map.has_method("get_missing_landmark_asset_count"), "layer map renderer should expose missing landmark count")
	var landmark_count: int = layer_map.get_landmark_object_count()
	var generated_count: int = layer_map.get_generated_landmark_asset_count()
	var missing_count: int = layer_map.get_missing_landmark_asset_count()
	_assert(generated_count + missing_count == landmark_count, "generated and missing landmark counts should cover all landmark slots")
	if missing_count > 0:
		_assert(layer_map.get_asset_generation_status() == "pending_tool_unavailable", "landmark art should stay pending while built-in imagegen output is missing")
		_assert(layer_map.get_pending_landmark_asset_count() == missing_count, "missing landmark art should be explicitly pending built-in imagegen")
	else:
		if current_source == "local_fallback":
			_assert(layer_map.get_asset_generation_status() == "generated_local_image_generator_fallback_complete", "landmark art should record local fallback completion after every generated PNG is loadable")
		else:
			_assert(layer_map.get_asset_generation_status() == "generated_builtin_imagegen_complete", "landmark art should record built-in completion after every generated PNG is loadable")
	if not previous_scene_id.is_empty():
		town_map.show_scene(previous_scene_id)
