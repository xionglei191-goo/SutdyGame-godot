...
func _initialize() -> void:
	...
	_assert(finish_reading_button.text == "完成 home-first Quest 和 Story Show 后可用", "empty parent summary should explain finish prerequisite")
	...
	dialogue_box._finish()
	await process_frame
	_assert(quest_diary.active, "home Mina intro should start Room Starter after Welcome Box")
	_assert(quest_diary.quest_id == "prologue_room_starter", "home Mina intro should hand off to Room Starter")
	_assert(quest_diary.event_label.text == "Room Starter", "Quest Diary should show the room starter event name")
	_assert(quest_diary.prompt_label.text == "Find your blue bag at home.", "Quest Diary should use room-object prompt")
	_assert(quest_diary.reward_label.text == "Keepsake: Room Starter Sticker", "Quest Diary should show the room starter keepsake")
	quest_diary.check_target("home_book")
	_assert_not_contains(_completed_quests, "prologue_room_starter", "wrong room target should not complete Room Starter")
	_assert(quest_diary.active, "Room Starter should remain active after wrong target")
	quest_diary.check_target("home_bag")
	await process_frame
	_assert_contains(_completed_quests, "prologue_room_starter", "Room Starter completion")
	_assert(game_state.coins == 5, "Room Starter should add reward_coins from quest data")
	_assert(mina.dialogue_id == "mina_pet_hello_intro", "Mina should switch to Pet Hello dialogue after Room Starter")

	mina.interaction_requested.emit(mina.dialogue_id)
	await process_frame
	dialogue_box._finish()
	await process_frame
	_assert(quest_diary.active, "home Mina intro should start Pet Hello after Room Starter")
	_assert(quest_diary.quest_id == "prologue_pet_hello", "home Mina intro should hand off to Pet Hello")
	_assert(quest_diary.event_label.text == "Pet Hello", "Quest Diary should show the pet hello event name")
	_assert(quest_diary.prompt_label.text == "Say hello to the pet corner.", "Quest Diary should use pet hello prompt")
	_assert(quest_diary.reward_label.text == "Keepsake: Pet Hello Tag", "Quest Diary should show the pet hello keepsake")
	quest_diary.check_target("home_pet_bowl")
	_assert_not_contains(_completed_quests, "prologue_pet_hello", "wrong pet target should not complete Pet Hello")
	quest_diary.check_target("home_pet_corner")
	await process_frame
	_assert_contains(_completed_quests, "prologue_pet_hello", "Pet Hello completion")
	_assert(game_state.get_pet_name() == "Coco", "Pet Hello should name the starter pet")
	_assert(game_state.coins == 6, "Pet Hello should add reward_coins from quest data")
	_assert(mina.dialogue_id == "mina_home_pet_care_intro", "Mina should switch to Home Pet Care dialogue after Pet Hello")

	mina.interaction_requested.emit(mina.dialogue_id)
	await process_frame
	dialogue_box._finish()
	await process_frame
	_assert(quest_diary.active, "home Mina intro should start Home Pet Care after Pet Hello")
	_assert(quest_diary.quest_id == "prologue_home_pet_care", "home Mina intro should hand off to Home Pet Care")
	_assert(quest_diary.event_label.text == "Home Pet Care", "Quest Diary should show the pet care event name")
	_assert(quest_diary.prompt_label.text == "Feed Coco before the first trip.", "Quest Diary should use pet care prompt")
	_assert(quest_diary.reward_label.text == "Keepsake: Pet Care Heart", "Quest Diary should show the pet care keepsake")
	feed_button.pressed.emit()
	await process_frame
	_assert_contains(_completed_quests, "prologue_home_pet_care", "Home Pet Care completion")
	_assert(game_state.coins == 6, "Home Pet Care should spend 2 coins and then add reward_coins")
	_assert(quest_diary.status_label.text == "Done", "Quest Diary should show done status after Home Pet Care")
	_assert(mina.dialogue_id == "mina_first_trip_handoff", "Mina should switch to First Trip dialogue after Home Pet Care")

	mina.interaction_requested.emit(mina.dialogue_id)
	await process_frame
	dialogue_box._finish()
	await process_frame
	_assert(quest_diary.active, "home Mina handoff should start First Trip after Home Pet Care")
	_assert(quest_diary.quest_id == "prologue_go_to_school", "home Mina handoff should hand off to First Trip")
	...
	_assert(game_state.coins == 7, "First Trip should add reward_coins from quest data")
	...
	_assert(game_state.coins == 9, "Walk With Mina should add reward_coins from quest data")
	...
	_assert(game_state.coins == 14, "Bird Watch should add reward_coins from quest data")
	...
	_assert(pre_review_finish_button.text == "完成 home-first Quest 和 Story Show 后可用", "parent summary should explain Story Show prerequisite before finish")
	...
	_assert(not game_state.has_story_flag(game_state.LEGACY_PARENT_BONUS_CONFIRM_FLAG), "new parent bonus confirmation should not rewrite the legacy flag")
	...
	_assert_contains(snapshot["completed_quests"], "prologue_room_starter", "saved Room Starter")
	_assert_contains(snapshot["completed_quests"], "prologue_pet_hello", "saved Pet Hello")
	_assert_contains(snapshot["completed_quests"], "prologue_home_pet_care", "saved Home Pet Care")
	_assert_contains(snapshot["completed_quests"], "prologue_go_to_school", "saved First Trip")
	...
	_assert(game_state.get_completed_quests().size() == 8, "load should restore 8 completed quests including the home prologue")
	...
	_assert_contains(game_state.get_completed_quests(), "prologue_room_starter", "loaded room starter quest")
	_assert_contains(game_state.get_completed_quests(), "prologue_pet_hello", "loaded pet hello quest")
	_assert_contains(game_state.get_completed_quests(), "prologue_home_pet_care", "loaded home pet care quest")
...
func _assert_parent_summary_completed(parent_summary: CanvasLayer) -> void:
	...
	var parent_bonus_button: Button = parent_summary.get_node("Panel/MarginContainer/VBoxContainer/ParentBonusButton")
	var export_report_button: Button = parent_summary.get_node("Panel/MarginContainer/VBoxContainer/ExportReportButton")
	_assert(completed_value.text == "8", "parent summary should show 8 completed quests including the home prologue")
	_assert(quests_value.text.contains("Welcome Box"), "parent summary should show starter quest name")
	_assert(quests_value.text.contains("Room Starter"), "parent summary should show room starter quest name")
	_assert(quests_value.text.contains("Pet Hello"), "parent summary should show pet hello quest name")
	_assert(quests_value.text.contains("Home Pet Care"), "parent summary should show home pet care quest name")
	...
	_assert(rewards_value.text.contains("Room Starter Sticker"), "parent summary should show room starter reward")
	_assert(rewards_value.text.contains("Pet Hello Tag"), "parent summary should show pet hello reward")
	_assert(rewards_value.text.contains("Pet Care Heart"), "parent summary should show pet care reward")
	...
	_assert(words_value.text.contains("bag"), "parent summary should show word record bag")
	_assert(words_value.text.contains("pet"), "parent summary should show word record pet")
	_assert(words_value.text.contains("feed"), "parent summary should show word record feed")
	...
	_assert(patterns_value.text.contains("This is my bag."), "parent summary should show room starter pattern")
	_assert(patterns_value.text.contains("Hello, Coco."), "parent summary should show pet hello pattern")
	_assert(patterns_value.text.contains("Feed the pet."), "parent summary should show pet care pattern")
