...
- `Memory Spark` 首访对话 / 回访提取试点，运行时已使用 `MemorySpark*` 命名；`anchor_*` ID 与 `anchor_recall_done_*` 存档 flag 保持兼容。
...
4. 运行时当前已经包含：
   - `world_overview`
   - `Welcome Box -> First Trip -> Walk With Mina -> Room Helper -> Bird Watch`
   - `home pet care`
