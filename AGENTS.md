...
- New games start at `HomeLayer` for the `Welcome Box` opener. `SceneHost.tscn` / `WorldOverviewScene.tscn` remains the world-map runtime scene opened from home and used for A-Z anchors, town exploration, PlaceCard, and transport/world routing. The current generated overview map is a large `2560x2560` square exploration surface shown through the `1280x720` runtime viewport.
